...
const Parents: React.FC<ParentsProps> = ({ setPage }) => {
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {[
            { title: "Physical Discipline", icon: "fa-person-running", desc: "Movement, self-regulation, breathwork.", link: Page.PHYSICAL_DISCIPLINE },
            { title: "Academic Competence", icon: "fa-book-open", desc: "Homework support, goal setting.", link: Page.ACADEMIC_COMPETENCE },
            { title: "Social Skill", icon: "fa-users", desc: "Conflict resolution, rhetoric, teamwork.", link: Page.SOCIAL_SKILL },
            { title: "Financial Literacy", icon: "fa-coins", desc: "Budgeting, 'padded' economy, value.", link: Page.FINANCIAL_LITERACY },
            { title: "Entrepreneurship", icon: "fa-rocket", desc: "Project work, ownership, bold execution.", link: Page.ENTREPRENEURSHIP },
          ].map((pillar, i) => (
            <div 
              key={i} 
              onClick={() => setPage(pillar.link)}
              className="bg-white p-8 rounded-2xl shadow-lg border-b-4 border-brand-navy hover:border-brand-gold transition-all duration-300 hover:-translate-y-2 group cursor-pointer"
            >
